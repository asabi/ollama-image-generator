...
- **Dynamic model discovery**: image and text models are detected via Ollama's `/api/show` capabilities, so any new image-gen model you pull shows up automatically. A small catalog of well-known names (z-image-turbo variants, flux2-klein variants, popular text models) appears in the dropdowns even if you haven't pulled them, and selecting one reveals an inline "Pull it now" button with a real progress bar.
